...
def parse_args():
    ...
    parser.add_argument('video_path', type=str, default='PATH_TO_DATA',
                        help='root directory for the input videos')
    parser.add_argument('out_path', type=str,default='PATH_TO_DATA',
...
def vid2jpg(tup, decode_type, se=None, fps=None, prefix='img_%05d.jpg'):
    ...
    src = path_remake(src)
    dest = path_remake(dest)
    video_folder = path_remake(video_folder)
    try:
    ...
    if decode_type == 'ffmpeg':
        ...
        import cv2
        src = path_origin(src)
        video_folder = path_origin(video_folder)
        #video_folder = path_remake(video_folder)
        cap = cv2.VideoCapture(src)
        ret, frame = cap.read()
        if ret:
