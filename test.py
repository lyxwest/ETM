...
def main(args):
    ...
    if os.path.isfile(args.weights):
        ...
        model_update = update_dict(checkpoint['model_state_dict'])
        model_full.load_state_dict(model_update, strict=True)
        del checkpoint
    ...
    if os.path.isfile(class_feats_file):
        ...
        model.eval()
        with torch.no_grad():
            classes_features = model.encode_text(classes)  # n_class feature_dim(512/768/..)
...
def validate(val_loader, device, model, config, text_features, test_crops, test_clips):
    ...
    with torch.no_grad():
        ...
        for i, (image, class_id) in enumerate(val_loader):
            ...
            video_features = model.module.encode_right_video(image)
            cnt_time = time.time() - proc_start_time
            video_features = video_features.reshape(batch_size, num_crop, -1).mean(1)  # bs dim
