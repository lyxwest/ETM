...
from modules.videoclip_etm import video_header, VideoCLIP
...
def best_saving(working_dir, epoch, model, optimizer):
    best_name = '{}/best_model.pt'.format(working_dir)
    torch.save({
        'epoch': epoch,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
...
def main(args):
    ...
    if dist.get_rank() == 0:
        ...
        shutil.copy('modules/videoclip_etm.py', working_dir)
        shutil.copy('scripts/run_train.sh', working_dir)
    ...
    config = DotMap(config)    #修改yaml的config格式
    ...
    for name, param in model_full.named_parameters():   
        ...
        if 'visual' in name and 'control_point' not in name:
            ...
            scl_params.append(param)
        else:
            other_params.append(param)

    for name, param in model_full.named_parameters():
        logger.info('{}: {}'.format(name, param.requires_grad))
    num_param = sum(p.numel() for p in model_full.parameters() if p.requires_grad)
    num_total_param = sum(p.numel() for p in model_full.parameters())
    ...
    optimizer = optim.AdamW([{'params': clip_params, 'lr': config.solver.lr * config.solver.clip_ratio}, 
                            {'params': scl_params, 'lr': config.solver.lr},
...
def train(model, train_loader, optimizer, criterion, scaler,
    ...
    for i,(images, list_id) in enumerate(train_loader):
        ...
        with autocast():
            contrastive_logits, ce_logits = model(images, text_embedding, list_id) # B 400

            contrastive_target = torch.zeros(contrastive_logits.shape[0], dtype=list_id.dtype).to(device)
            ce_loss = criterion(ce_logits, list_id.to(device))
            contrastive_loss = criterion(contrastive_logits, contrastive_target)
        ...
        losses.update(loss.item(), ce_logits.size(0))        
        ...
        if i % config.logging.print_freq == 0:
            logger.info(('Epoch: [{0}][{1}/{2}], lr: {lr:.2e}, eta: {3}\t'
                         ...
                         'CE Loss {ce_loss:.4f} \t'
                         'Con Loss {contrastive_loss:.4f} \t'
                         'Loss {loss.val:.4f} ({loss.avg:.4f})'.format(
                             epoch, i, len(train_loader), eta_sec, batch_time=batch_time, data_time=data_time, 
                             ce_loss=ce_loss.item(), contrastive_loss=contrastive_loss.item(),loss=losses,
...
def validate(epoch, val_loader, device, model, config, text_embedding, logger):
    ...
    with torch.no_grad():
        for i, (image, class_id) in enumerate(val_loader):
            ...
            video_embedding = model.module.encode_right_video(image)
